...
> Phase 0 abstract example. Variants by scenario with motion/segment section labels.

## Cold outbound  {motion: sales-assisted, segment: mid-market}

> `<2-3 sentence pitch optimized for cold email or LinkedIn outreach>`
>
> Lead with a specific pain the target persona has; tie to a specific outcome; one proof point with a named customer or specific metric.

## First call opener  {motion: sales-assisted}

> `<3-4 sentence pitch for first discovery call>`
>
> Positions quickly, then pivots to discovery. The goal is not to sell, but to earn the right to ask questions.

## Demo lead-in  {motion: sales-assisted}
