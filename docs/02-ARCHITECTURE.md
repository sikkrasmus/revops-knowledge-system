...
Changes land via PR. v1 ships the artifact-commit path: humans and LLMs edit via PR. The abstract connector pattern in [`../ingestion/connectors/PATTERN.md`](../ingestion/connectors/PATTERN.md) specifies five lifecycle stages (auth, extract, synthesize, snapshot, PR). Real implementations are built per-fork or forked from community contributions.
